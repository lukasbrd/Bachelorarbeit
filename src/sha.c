...
/* $Id: sha.c,v 1.1 2002/01/28 11:03:44 mishan Exp $ */
...
void sha1(char* input, int len, char* digest) {
    struct sha_ctx ctx;
    sha_init(&ctx);
    sha_update(&ctx,(uint8_t*)input,len);
...
static void sha_block(struct sha_ctx *ctx, uint8_t *block)
{
...
void sha_update(struct sha_ctx *ctx, uint8_t *buffer, uint32_t len)
{
  if (ctx->index)
...
void sha_digest(struct sha_ctx *ctx, uint8_t *s)
{
  int i;
...
void sha_final(uint8_t *s, struct sha_ctx *ctx)
{
  uint32_t data[SHA_DATALEN];
  int i;
  int words;
...
int
sha_fd (int fd, size_t maxlen, uint8_t *md)
{
    struct sha_ctx ctx;
#define BLOCKSIZE 4096
    uint8_t buffer[BLOCKSIZE + 72];
    size_t sum;
